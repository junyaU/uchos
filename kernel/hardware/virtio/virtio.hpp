...
constexpr size_t calc_driver_ring_size(size_t num_desc) {
  return num_desc * 2 + 6;
}

constexpr size_t calc_device_ring_size(size_t num_desc) {
  return num_desc * 8 + 6;
}

error_t init_virtio_pci_device(VirtioPciDevice *virtio_dev, int device_type);

error_t init_virtqueue(VirtioVirtqueue *queue, size_t index, size_t num_desc,
                       uintptr_t desc_addr, uintptr_t driver_ring_addr,
                       uintptr_t device_ring_addr);

int push_virtio_entry(VirtioVirtqueue *queue, VirtioEntry *entry_chain,
                      size_t num_entries);

int pop_virtio_entry(VirtioVirtqueue *queue, VirtioEntry *entry_chain,
                     size_t num_entries);
