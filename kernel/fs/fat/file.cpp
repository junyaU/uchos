...
error_t process_file_read_request(const Message &m, DirectoryEntry *entry,
                                  bool for_user) {
  char file_name[12] = {0};
...
void handle_fs_write(const Message &m) {
  ...
  if (needed_clusters > current_clusters) {
    ...
    if (count_free_clusters() < clusters_to_add) {
      LOG_ERROR("disk full: need {} clusters but only {} free", clusters_to_add,
                count_free_clusters());
      reply.data.fs.len = 0;
      kernel::task::send_message(m.sender, reply);
      return;
  ...
  for (size_t i = 0;
       i < cluster_offset && target_cluster != END_OF_CLUSTER_CHAIN; i++) {
    target_cluster = next_cluster(target_cluster);
  }
  ...
  void *cluster_buffer;
  ALLOC_OR_RETURN(cluster_buffer, BYTES_PER_CLUSTER,
                  kernel::memory::ALLOC_ZEROED);
  ...
  send_write_req_to_blk_device(
      cluster_buffer, calc_start_sector(target_cluster), BYTES_PER_CLUSTER,
      MsgType::FS_WRITE, m.data.fs.request_id);
