...
const InitialTaskInfo INITIAL_TASKS[] = {
    {"main", 0, false, true},
    {"idle", reinterpret_cast<uint64_t>(&kernel::task::idle_service), true,
     true},
    {"usb_handler",
     reinterpret_cast<uint64_t>(&kernel::task::usb_handler_service), true,
     true},
    {"virtio_blk",
     reinterpret_cast<uint64_t>(&kernel::hw::virtio::virtio_blk_service), true,
     false},
    {"fat32", reinterpret_cast<uint64_t>(&kernel::fs::fat32_service), true,
     true},
...
Task *create_task(const char *name, uint64_t task_addr, bool setup_context,
                  bool is_init) {
...
error_t Task::copy_parent_stack(const Context &parent_ctx) {
  ...
  auto parent_stack_top =
      reinterpret_cast<uint64_t>(parent->stack) + stack_size;
...
Task::Task(int raw_id, const char *task_name, uint64_t task_addr,
           TaskState state, bool setup_context, bool is_initilized)
    : id{ProcessId::from_raw(raw_id)}, parent_id{ProcessId::from_raw(-1)},
      priority{2}, // TODO: Implement priority scheduling
      is_initilized{is_initilized}, state{state},
      fs_path({nullptr, nullptr, nullptr}), stack{nullptr},
  ...
  kernel::memory::page_table_entry *page_table =
      kernel::memory::new_page_table();
